...
class DLoader:
    def __init__(self, args):
        ...
        spk_list = sorted(
            list(set([os.path.basename(wav).split("_")[0] for wav in wavs]))
        )[-8:]
    ...
    def __getitem__(self, index):
        ...
        audio, _ = librosa.effects.trim(
            audio, top_db=20, frame_length=2048, hop_length=600
        )

        if len(audio) % 600 != 0:
            audio = np.pad(
                audio, (0, 600 - (len(audio) % 600)), "constant", constant_values=0
            )

        basename = os.path.splitext(os.path.basename(self.wavs[index]))[0].replace(
            "_mic1", ""
        )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(d_loader)):
            ...
            for sr in sampling_rates:
                ...
                d_audio = librosa.resample(
                    d_audio, orig_sr=orig_sr, target_sr=sr, res_type="kaiser_best"
                )
                d_audio = librosa.resample(
                    d_audio, orig_sr=sr, target_sr=target_sr, res_type="kaiser_best"
                )
                ...
                src_wav = os.path.join(
                    args.output_dir, "source_wavs", str(sr), basename + ".wav"
                )
                gt_wav = os.path.join(args.output_dir, "gt_wavs", basename + ".wav")
                src_mel = os.path.join(
                    args.output_dir, "source_mels", str(sr), basename + ".pt"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_dir", required=True, help="Directory of audio files"
    )
    parser.add_argument(
        "-o", "--output_dir", required=True, help="Directory to save preprocessed data"
    )
    parser.add_argument(
