...
def get_hparams_pt(init=True):
    ...
    parser.add_argument(
        "-r", "--mask_ratio", type=float, required=True, help="Mask ratio"
    )
...
def get_hparams_ft(init=True):
    ...
    parser.add_argument("-m", "--model", type=str, required=True, help="Model name")
    parser.add_argument(
        "-p", "--pre_trained", type=str, default=None, help="pre-trained model path"
    )
    ...
    if args.pre_trained:
        if args.step:
            hparams.ckpt_file = os.path.join(
                "./logs/pretrain", args.pre_trained, "G_{}.pth".format(args.step)
            )
        else:
            hparams.ckpt_file = os.path.join(
                "./logs/pretrain", args.pre_trained, "G_{}.pth".format(32)
            )
    else:
...
def get_hparams2(init=True):
    ...
    parser.add_argument(
        ...
        "--config",
        type=str,
        default="./configs/pretrain_exp.json",
        help="JSON file for configuration",
    )
    parser.add_argument("-m", "--model", type=str, required=True, help="Model name")
    parser.add_argument(
    ...
    if args.pre_trained:
        if args.step:
            hparams.ckpt_file = os.path.join(
                "./logs/AudioMAE", args.pre_trained, "G_{}.pth".format(args.step)
            )
        else:
            hparams.ckpt_file = os.path.join(
                "./logs/AudioMAE", args.pre_trained, "G_{}.pth".format(32)
            )
    else:
        hparams.ckpt_file = None
    return hparams
...
class MelSpectrogram(torch.nn.Module):
    def __init__(
        self, sample_rate, n_fft, win_length, hop_length, f_min, f_max, n_mels
    ):
